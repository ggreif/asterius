...
asteriusIservCall hsc_env _ msg = do
  ...
  case msg of
    ...
    GHC.LoadArchive p -> modifyMVar_ globalGHCiState $ \s -> do
      ...
      evaluate s {ghciLibs = mempty{repMetadata = lib} <> ghciLibs s}
          ...
          linkExeInMemory
            LinkTask
              { progName = "",
                ...
                linkObjs = [],
                linkLibs = [],
                linkModule = fromAsteriusRepModule $ m <> M.foldr' (<>) (ghciLibs s) (ghciObjs s),
                hasMain = False,
                debug = False,
                gcSections = True,
                verboseErr = True,
    ...
    pure
      ( s
          { ghciCompiledCoreExprs =
              IM.insert this_id (sym, mempty{inMemoryModule = m}) $
